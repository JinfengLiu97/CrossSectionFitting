...
//For distance itself, double gaus will be used 
//For significance, convolution of gaus and exponent will be used
...
void Prompt() {
	...
	RooRealVar x1_("x1_", "c#tau^{J/#psi1} [cm]", -0.01, 0.015);
	...
	//ctau
	//Prompt (double gaus)
	RooRealVar ctau_Gaus_Mean("ctau_Gaus_Mean", "ctau_Gaus_Mean", 0, -0.006, 0.006);

	RooRealVar ctau_Gaus_Sigma1("ctau_Gaus_Sigma1", "ctau_Gaus_Sigma1", 0.003, 0.001, 0.01);
	RooRealVar ctau_Gaus_Sigma2("ctau_Gaus_Sigma2", "ctau_Gaus_Sigma2", 0.005, 0.001, 0.02);

	RooRealVar ctau_Gaus_Frac("ctau_Gaus_Frac", "ctau_Gaus_Frac", 0.5, 0, 1);

	RooAbsPdf* ctau_Gaus1 = new RooGaussian("ctau_Gaus1", "ctau_Gaus1", x1_, ctau_Gaus_Mean, ctau_Gaus_Sigma1);
	RooAbsPdf* ctau_Gaus2 = new RooGaussian("ctau_Gaus2", "ctau_Gaus2", x1_, ctau_Gaus_Mean, ctau_Gaus_Sigma2);

	RooAddPdf ctau_Gaus("ctau_Gaus", "ctau_Gaus", RooArgList(*ctau_Gaus1, *ctau_Gaus2), ctau_Gaus_Frac);
	...
	//Data acquire
	TFile F_input("~/work1/jinfeng/CrossSection/CMSSW_10_2_5/src/PreSelection/Feb/PreSelection/MC/NoVertex/Mixing/Oct/Prompt.root", "read");
	...
	//Fit
	RooFitResult* ctau_FitResult = ctau_Gaus.fitTo(*Set, Hesse(kTRUE), Strategy(1), NumCPU(4), Save(kTRUE), Minos(kFALSE));

	//Plot
	RooPlot* ctau_Frame = x1_.frame(Title("c#tau^{J/#psi1}"));

	//Line
	Set->plotOn(ctau_Frame, MarkerColor(kBlack), LineColor(kBlack), Name("Dat"));

	ctau_Gaus.plotOn(ctau_Frame, LineColor(kBlue), Name("Total"));
	
	//Pull
	RooHist* ctau_Pull = ctau_Frame->pullHist();
	RooPlot* ctau_Pull_Frame = x1_.frame(Title(" "));
	...
	*/
	...
	ctau_Canvas->SaveAs("Prompt_ctau_Result.pdf");


	//Print the result
	cout << "ctau(prompt): " << ctau_FitResult->status() << endl;
	cout << "NLL: " << ctau_FitResult->minNll() << endl;
	cout << "Chi2: " << ctau_Frame->chiSquare() << endl;
	cout << "Mean: " << ctau_Gaus_Mean.getValV() << " +/- " << ctau_Gaus_Mean.getError() << endl;
	cout << "Sigma1: " << ctau_Gaus_Sigma1.getValV() << " +/- " << ctau_Gaus_Sigma1.getError() << endl;
	cout << "Sigma2: " << ctau_Gaus_Sigma2.getValV() << " +/- " << ctau_Gaus_Sigma2.getError() << endl;
	cout << "Fraction: " << ctau_Gaus_Frac.getValV() << " +/- " << ctau_Gaus_Frac.getError() << endl;
